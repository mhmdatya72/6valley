...
class OrderController extends BaseController
{
    ...
    public function __construct(
        ...
        private readonly CouponRepositoryInterface                        $couponsRepo,
    ...
    {
        // Fetch the order with its related data using the repository

        $order = $this->orderRepo->getFirstWhere(params: ['id' => $id], relations: ['details', 'shipping', 'seller']);
        // Fetch linked orders using the repository
        ...
        // Fetch active coupons using the repository
        $today = date('Y-m-d');
        $coupons = $this->couponsRepo->getListWhere(
            filters: ['start_date' => '<=', 'expire_date' => '>='],
            dataLimit: 'all'
        );

        // Fetch shipping method and delivery men using the repository
        $shippingMethod = $this->businessSettingRepo->getFirstWhere(params: ['type' => 'shipping_method'])->value;
        $deliveryMen = $this->deliveryManRepo->getListWhere(
            filters: [
                'is_active' => 1,
                'seller_id' => $order->seller_is == 'admin' ? 0 : ($order->seller_is == 'seller' && $shippingMethod == 'sellerwise_shipping' ? $order->seller_id : 0)
            ],
            dataLimit: 'all'
        );

        // Fetch products based on category using the repository
        $products = $this->productRepo->getListWhere(
            filters: ['status' => 1, 'category_ids' => $request->has('category_id') && $request->category_id != 0 ? [$request->category_id] : null],
            dataLimit: 'all'
        );

        // Fetch shipping address using the repository
        $shippingAddress = $this->shippingAddressRepo->getFirstWhere(params: ['id' => $order->shipping_address]);

        // Render view based on order type
        if ($order->order_type == 'default_type') {
            return view('admin-views.order.list', compact('coupons', 'shippingAddress', 'order', 'linkedOrders', 'deliveryMen', 'products'));
        } else {
            return view('admin-views.order.list', compact('order', 'products'));
        }
    }
    ...
    public function edit(Request $request, $id)
    {
        $order = $this->orderRepo->getFirstWhere(
            params: ['id' => $id],
            relations: ['details', 'shipping', 'seller']
        );
        $physicalProduct = $this->productRepo->getFirstWhere(
            params: ['id' => $request->input('product_id')]
        );
        ...
        $shippingMethod = $this->businessSettingRepo->getFirstWhere(
            params: ['type' => 'shipping_method']
        )->value;

        $deliveryMen = $this->deliveryManRepo->getListWhere(
            filters: [
                'is_active' => 1,
                'seller_id' => $this->getSellerId($order, $shippingMethod)
            ],
            dataLimit: 'all'
        );
        ...
        $products = $this->productRepo->getListWhere(
            filters: [
                'status' => 1,
                'category_ids' => $request->filled('category_id') && $request->category_id != 0
                    ? [$request->category_id]
                    : null
            ],
            dataLimit: 'all'
        ...
        $shippingAddress = $this->shippingAddressRepo->getFirstWhere(
            params: ['id' => $order->shipping_address]
        );
        $companySettings = $this->businessSettingRepo->getFirstWhere(
            params: ['type' => 'company_logo']
        );
        $companyWebLogo = $companySettings ? $companySettings->value : 'default_logo.png';
        $companySettings = $this->businessSettingRepo->getFirstWhere(
            params: ['type' => 'company_name']
        );
        $companyName = $companySettings ? $companySettings->value : 'Default Company Name';
       $isOrderOnlyDigital = $order->details->every(function($detail) {
        return $detail->product->type === 'digital';
    });
        return $this->renderOrderUpdateView($order, compact('isOrderOnlyDigital','companyName','companyWebLogo','order','physicalProduct','coupons', 'shippingAddress', 'linkedOrders', 'deliveryMen', 'products'));
    }


        protected function getSellerId($order, $shippingMethod)
        {
        ...
        protected function renderOrderUpdateView($order, $data)
        {
            if ($order->order_type === 'default_type') {
                return view('admin-views.order.order-update', $data);
            }
