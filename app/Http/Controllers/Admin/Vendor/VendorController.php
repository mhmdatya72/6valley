...
//    }
   ...
   public function add(VendorAddRequest $request): JsonResponse
{
    // Add payment_method to the vendor data
    $vendorData = $this->vendorService->getAddData($request);
    // $vendorData['payment_method'] = $request->input('payment_method');
    $vendorData['payment_method'] = $request->input('payments');
    $vendorData['holidays'] = $request->input('days');
    $vendorData['shipping_time'] = $request->input('shipping_time');
    $vendorData['returned'] = $request->input('returned');
    $vendorData['minimum_order_amount'] = $request->input('minimum_order_amount');


    // Save vendor data
    $vendor = $this->vendorRepo->add(data: $vendorData);

    // Additional logic to handle shop and wallet data
    $this->shopRepo->add($this->shopService->getAddShopDataForRegistration(request: $request, vendorId: $vendor['id']));
    $this->vendorWalletRepo->add($this->vendorService->getInitialWalletData(vendorId: $vendor['id']));
