...
class Helpers
{
    ...
    public static function get_customer($request = null)
    {
     ...
     public static function coupon_discount_for_web($coupon_code)
    {
    ...
    public static function coupon_discount2($request)
    {
        $discount = 0;
    ...
    public static function rating_count2($p, $rating)
    {
        return Review::whereIn('product_id', $p)->where(['rating' => $rating])->count();
    }
    ...
    {
        $config = null;
        $check = ['currency_Models', 'currency_symbol_position', 'system_default_currency', 'language', 'company_name', 'decimal_point_settings','product_brand','digital_product', 'company_email', 'recaptcha'];
    ...
    public static function set_data_format($data)
    {
        ...
        foreach ($variation_arr as $var) {
            if(in_array($var['type'] , $variationName)) continue;
            $variationName [] = $var['type'];
            $variation[] = [
                'type' => $var['type'],
                'price' => (double)$var['price'],
                'sku' => $var['sku'],
                'qty' => (integer)($data['current_stock'] / $var['numberOfPieces']),
                "purchase_price" => $var['purchase_price'],
                "tax" => $var['tax'],
                "discount" =>(double) $var['discount'],
                "discount_type" => $var['discount_type'],
                "tax_type" => $var['tax_type'],
                "shipping_cost" => $var['shipping_cost'],
                "multiply_qty" => $var['multiply_qty'],
                "numberOfPieces" => $var['numberOfPieces'],
    ...
    {
        $currency_Models = Helpers::get_business_settings('currency_Models');
        if ($currency_Models == 'multi_currency') {
            if (session()->has('usd')) {
                $usd = session('usd');
            } else {
